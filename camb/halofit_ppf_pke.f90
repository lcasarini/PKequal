...
!P(k) equivalence Casarini, Bonometto e Macció (2009) JCAP 3,14 
!             and Casarini, Bonometto, Tessarotto & Corasaniti (2016) JCAP, ?, ?  
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      private
       ...
       real(dl):: error,dlsb,dlsb_eq,z_star,tau_star,w_true,wa_true,w_hf,wa_hf=0._dl !lc
...
! P(k) equivalence Casarini, Bonometto e Macció (2009)
       z_star=ThermoDerivedParams( derived_zstar )
       tau_star=TimeOfz(z_star)
       dlsb=TimeOfz(CAMB_Pk%Redshifts(itf))-tau_star
       w_true=w_lam
       wa_true=wa_ppf
       wa_ppf=0._dl
       do 
          z_star=ThermoDerivedParams( derived_zstar )
          tau_star=TimeOfz(z_star)
          dlsb_eq=TimeOfz(CAMB_Pk%Redshifts(itf))-tau_star
          error=1.d0-dlsb_eq/dlsb
          if (abs(error).le.1e-7) goto 91
 ...
 91    continue   
       ...
       w_lam=w_true
       wa_ppf=wa_true 
       write(*,*)'at z = ',real(CAMB_Pk%Redshifts(itf)),' equivalent const. w=', real(w_hf)   
...
!cosmological models with constant w.
      ...
      a=1.5222+2.8553*rn+2.3706*rn*rn+0.9903*rn*rn*rn+ &
           0.2250*rn*rn*rn*rn-0.6038*rncur+0.1749*om_v*(1.+w_hf)
      a=10**a      
